...
- **`/whitelist [days] [user] `** - Grants the specified user access to the chosen script for X days. Run command, then select the script from the dropdown.

- **`/trial [hours] [user]`**  - Issues short‑term access (in hours) similar to a whitelist. Run command, then select the script from the downdown.


> **Note:** Only users with “Manage Server” or “Administrator” can run these admin commands.
